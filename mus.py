		...
		if songdata[0].strip() != "":
			...
			if songdata[2].strip() != "":
				...
				ppl = [info.strip()[1:] if info.strip()[0]=="\\" else info.strip()\
						for info in songdata[3:]\
						if len(info.strip())>1 and info.strip()[0]!=":"]
				artistmeta = [info.strip()[1:] for info in songdata[3:]\
								if len(info.strip())>0 and info.strip()[0]==":"]
				if (len(artistmeta)==0):
					artistmeta.append("o")
				elif (len(artistmeta)>1):
					if not quiet:
						err = "Multiple instances of artist metainfo:"
						ifprint("{} {}".format(err, line))
...
def main():
	...
	while (not done):
		try:
			...
			print()
			sys.exit();
		args = re.split(r"(?<!\\)\s+", inp)
		args = [re.sub(r"(\\) "," ", arg).lower() for arg in args]
		if (len(args) != 0):
			cmd = args[0]
			if (cmd in ('quit','exit')):
				done = True
			elif (cmd == 'disc'):
				if (len(args)==1 and not quiet):
					print("disc command requires argument(s)")
				else:
					arg = " ".join(args[1:]).lower()
					if arg in artists:
						artist = artists[arg]
					else:
						matches = [name for name in artists if arg in name]
						if len(matches)==1:
							artist = artists[matches[0]]
						else:
							
					print("-"*32)
					print(" "*((32-len(artist.name))//2) + artist.name)
					print("-"*32)
					for title in artist.songs:
						song, role = artist.songs[title]
						print("{} {}".format(role, song.title))
					print()
